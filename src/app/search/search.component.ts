...
export class SearchComponent {
  ...
  applyFilters() {
    ...
    if (cityQ)
      rows = rows.filter((r) => (r.city ?? '').toLowerCase().includes(cityQ));
    if (streetQ)
      rows = rows.filter((r) =>
        (r.streetName ?? '').toLowerCase().includes(streetQ)
      );
    ...
    if (pMin != null)
      rows = rows.filter((r) => Number(r.rentPrice ?? 0) >= pMin);
    if (pMax != null)
      rows = rows.filter((r) => Number(r.rentPrice ?? 0) <= pMax);
    if (aMin != null)
      rows = rows.filter((r) => Number(r.areaSize ?? 0) >= aMin);
    if (aMax != null)
      rows = rows.filter((r) => Number(r.areaSize ?? 0) <= aMax);
  ...
  private sortRows(
    ...
    switch (key) {
      case 'priceAsc':
        rows.sort(
          (a, b) => Number(a.rentPrice ?? 0) - Number(b.rentPrice ?? 0)
        );
        break;
      case 'priceDesc':
        rows.sort(
          (a, b) => Number(b.rentPrice ?? 0) - Number(a.rentPrice ?? 0)
        );
        break;
      case 'cityAsc':
        rows.sort((a, b) =>
      ...
      case 'cityDesc':
        ...
        break;
      case 'areaAsc':
        rows.sort((a, b) => Number(a.areaSize ?? 0) - Number(b.areaSize ?? 0));
        break;
      case 'areaDesc':
        rows.sort((a, b) => Number(b.areaSize ?? 0) - Number(a.areaSize ?? 0));
        break;
    }
    return rows;
  }
